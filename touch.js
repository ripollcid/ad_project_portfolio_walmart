...
(function(){
	...
	var touchBase = {
		...
		overLimit: function(change, axis) {
			return Math.abs(change) > this[axis + 'Threshold'];
		},
		...
		getTouchInfo: function(touches) {
			...
			o.maxTouches = Math.max(touches.length, o.maxTouches);
			if (o.maxTouches === 1) {

				focus = p.singleSwipe = p.singleSwipe || {
					fingers: [{
				...
				focus = p.doubleSwipe = p.doubleSwipe || {
					fingers: [{
						startX: touches[0].pageX,
						startY: touches[0].pageY
					}, {
		...
		checkForEvents: function(focus) {
			...
			if (o.maxTouches === 1) {
				...
				if (o.overLimit(chiny, 'y') && o.gradCheck(gradient, 'v')) {
					eventType.push(chiny > 0 ? 'swipingUp' : 'swipingDown');
				} else if (o.overLimit(chinx, 'x') && o.gradCheck(gradient, 'h')) {
				...
				if (!focus.pinched) {
					...
					if (o.overLimit((Math.sqrt((chiny*chiny) + (chinx*chinx))), 'd')) {
		...
		touchEnd: function(e) {
			...
			var reset = function() {
				o.maxTouches = 0;
				p.singleSwipe = undefined;
				p.doubleSwipe = undefined;
			};
			var eventType = [];
			...
			if (o.maxTouches === 1 && e.touches.length === 0) {
				...
				singleDirection = !(o.overLimit(yDispVsDist, 'yEnd') || o.overLimit(xDispVsDist, 'xEnd'));

				if (o.overLimit(chiny, 'y') && o.gradCheck(gradient, 'v') && !o.overLimit(yDispVsDist, 'y') && singleDirection) {
					eventType.push(chiny > 0 ? 'swipeUp' : 'swipeDown');
				} else if (o.overLimit(chinx, 'x') && o.gradCheck(gradient, 'h') && !o.overLimit(xDispVsDist, 'x') && singleDirection) {
					eventType.push(chinx > 0 ? 'swipeLeft' : 'swipeRight');
				} else if (!o.overLimit(chiny, 'y') && !o.overLimit(chinx, 'x')) {
					eventType.push('tap');
				}
				...
				if (!gestured) {
					if (o.overLimit(chiny, 'y') && o.gradCheck(gradient, 'v') && singleDirection) {
						eventType.push(chiny > 0 ? 'dragUp' : 'dragDown');
					} else if (o.overLimit(chinx, 'x') && o.gradCheck(gradient, 'h') && singleDirection) {
						eventType.push(chinx > 0 ? 'dragLeft' : 'dragRight');
					}

					if(!o.overLimit(chiny, 'y') && !o.overLimit(chinx, 'x')) {
	...
	window.TOUCHJS = {
		...
		setThresholds: function(props) {
			...
			// }
			if(props.swiping) {
				t.xThreshold = props.swiping.x || 20;
				t.yThreshold = props.swiping.y || 20;
			}
			if(props.swipe) {
				t.xEndThreshold = props.swipe.x || 40;
				t.yEndThreshold = props.swipe.y || 40;	
			}
			if(props.pinch) {
				t.pThreshold = props.pinch;
			}
			if(props.gradient) {
				t.gradThreshold = props.gradient;
			}
		}
	};
